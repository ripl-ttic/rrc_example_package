...
class CubeManipulator:
    def __init__(self, env):
        self.viz = Viz() if env.visualization else None
        self.env = env
    ...
    def calc_yaw_tip_positions(self, obs):
        from code.wholebody_planning import WholeBodyPlanner
    ...
    def motion_planning(self, cube_tip_positions, obs):
        ...
        for ind in inds:
            ...
            target_joint_angles = pinocchio_solve_ik(
                self.env.pinocchio_utils.inverse_kinematics,
                obs,
    ...
    def _run_planned_actions(self, obs, action_seq, action_type, teleport=False, frameskip=2):
        ...
        if teleport:
            ...
            obs['robot_tip_positions'] = self.env.pinocchio_utils.forward_kinematics(action_seq[-1])
            return obs
