...
import time
from code.utils import apply_transform, CylinderMarker, VisualCubeOrientation
from trifinger_simulation import TriFingerPlatform
from code.env.cube_env import ActionType
from gym.wrappers import TimeLimit
import gym
from code.grasping import Cube, CoulombFriction, Transform, CubePD, get_rotation_between_vecs, PDController
from code.utils import set_seed, action_type_to
import argparse
...
class ForceControlPolicy(object):
    def __init__(self, env, apply_torques=True, tip_pd=None, mu=1.0,
        ...
        if env.simulation:
            ...
            force_gains = [e * 6 for e in default_force_gains]
            torque_gains = [e * 6 for e in default_torque_gains]
            max_force = 4 * default_max_force
            max_torque = 4 * default_max_torque
    ...
    def _calc_jacobians(self, obs):
        Js = [self.env.pinocchio_utils.compute_jacobian(
              i, obs['robot_position']) for i in range(3)]
        return Js


def grasp_tippos_control(env, obs):
    ik = env.pinocchio_utils.inverse_kinematics
    ...
    env.unwrapped.action_type = ActionType.TORQUE_AND_POSITION
    for i in range(10):
...
def run_episodes(neps, seed):
    ...
    for _ in range(neps):
        ...
        env.unwrapped.action_type = ActionType.TORQUE
        done = False
        while not done:
